...
int lowest_percentage(STUDENT *data, int n)
{
    ...
    for (int i = 0; i < n; i++)
        if (data[i].percentage < min)
            min = data[i].percentage;
    return min;
}
...
double std_deviation(STUDENT *data, int n)
{
    ...
    for (int i = 0; i < n; i++)
        sum_deviation_square += pow((data[i].percentage - mean(data, n)), 2);
